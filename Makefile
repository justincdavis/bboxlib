...
clean: 
	...
	rm -rf dist
	rm -rf *.egg-info
	rm -rf bboxlib/*.egg-info
	rm -rf src/bboxlib/*.egg-info
	pyclean .
	rm -rf .mypy_cache
	rm -rf .ruff_cache
...
docs:
	...
	rm -rf docs/source/*
	sphinx-apidoc -o docs/source/ src/bboxlib/
	cd docs && make html
...
codecs:
	python3 scripts/parse_codec.py --file=data/codecs.html --output=src/bboxlib/io/_fourcc.py

ci: ruff mypy

mypy:
	python3 -m mypy src/bboxlib --config-file=pyproject.toml

pyright:
	python3 -m pyright --project=pyproject.toml

ruff:
	python3 -m ruff format ./src/bboxlib
	python3 -m ruff check ./src/bboxlib --fix --preview
